...
coelho_x, coelho_z = 0.0, 80.0 # Nasce mais pra baixo agora
coelho_dir = 0.0
coelho_anda = False
COELHO_R = 8
COELHO_V = 1.8 
...
# Layout da Vila - Deslizados em Z (+15) para dar respiro ao rio
casas = [
    ...
    {"x":115,"z": 45,"cor":(110,220,130),"e":0.56,"r":-1.5708,"ch":True, "sh":(122,162,132), "smoke":"soft"}, # Casa 2 agora com chaminé (ch: True)
    {"x":-75,"z": 80,"cor":(245,175,70), "e":0.58,"r":0,      "ch":True, "sh":(188,148,108), "smoke":"normal"}, 
]

ruas = [
    {"cx":-15,"cz":7,"w":220,"d":28}, 
...
# Vegetação deslocada +15 Z
arvores = [
    {"x":-135,"z":40, "h":38}, {"x":-128,"z":110,"h":32},
...
arbs = [
    {"x":-128,"z":-23,"s":12}, {"x":65,"z":-20,"s":10},
...
class Particula:
    def __init__(self, x, y, z, tipo="normal"):
        ...
        self.vida = 1.0
        self.dec = random.uniform(0.01, 0.018)
        self.tam = random.uniform(3, 5)
        self.tipo = tipo # Nova flag para fumaça suave
...
class Abelha:
    def __init__(self, cx, cz):
        self.cx = cx + random.uniform(-15, 15)
        self.cz = cz + random.uniform(-15, 15)
        self.fase = random.uniform(0, 6.28)
        self.raio = random.uniform(8, 18)
        self.vel = random.uniform(0.003, 0.008)
        self.alt = random.uniform(14, 26)
        self.wv = random.uniform(0.4, 0.7) 
...
def setup():
    ...
    for c in casas:
        hw, hd = 60 * c["e"], 50 * c["e"]
        if abs(abs(c["r"]) - 1.5708) < 0.3: hw, hd = hd, hw
    ...
    for c in casas:
        ...
        if c["ch"]:
            ...
            px = c["x"]
            py = CHAO_Y - 40*e - 65*e # Sai difuso do meio do telhado se não houver chaminé
            pz = c["z"]
...
def draw():
    ...
    if mouseIsPressed:
        if mouseX > width - 70:
            ...
            if dist(mouseX, mouseY, 50, height/2 - 40) < 20: ESCALA = min(2.5, ESCALA + 0.02)
            if dist(mouseX, mouseY, 50, height/2 + 20) < 20: ESCALA = max(0.8, ESCALA - 0.02)
            if dist(mouseX, mouseY, 25, height - 80) < 25: giro_camera += 0.03
            if dist(mouseX, mouseY, 75, height - 80) < 25: giro_camera -= 0.03

    ciclo = remap(tempo_dia, 0, 1, PI/2, 3*PI/2)
    alt_sol = max(-1.0, min(sin(ciclo), 0.94))

    C_DIA=(140, 205, 230); C_TAR=(255, 130, 85); C_NOI=(18, 28, 55)
    L_DIA=(255, 248, 210); L_TAR=(255, 155, 85); L_NOI=(22, 32, 65) 
    ...
    if alt_sol > 0:
        inte = max(0, min(1, remap(alt_sol, 0, 0.15, 0, 1)))
        inte = inte * 0.75 
    ...
    mover_coelho_teclado()

    # Passando o tipo de fumaça da chaminé para a partícula gerada
    for cp in chamine_pos:
        if random.random() < 0.12: fumaca.append(Particula(cp["x"], cp["y"], cp["z"], cp["tipo"]))
    for p in fumaca:
        p.y -= p.vy; p.x += p.vx; p.z += p.vz; p.vida -= p.dec; p.tam += 0.07
    fumaca = [p for p in fumaca if p.vida > 0]
...
# =========================================================
# LÓGICA DO COELHO VIA TECLADO (CAMERA-RELATIVE)
# =========================================================
def mover_coelho_teclado():
    ...
    # Controles base
    if keyIsDown(UP_ARROW):    bx -= COELHO_V; andando = True
    if keyIsDown(DOWN_ARROW):  bx += COELHO_V; andando = True
    if keyIsDown(LEFT_ARROW):  bz += COELHO_V; andando = True
    if keyIsDown(RIGHT_ARROW): bz -= COELHO_V; andando = True

    if andando:
        coelho_anda = True
        
        # Rotaciona o vetor de movimento de acordo com o giro da câmera
        ca = cos(giro_camera)
        sa = sin(giro_camera)
...
def desenha_riacho():
    ...
    for i in range(8):
        ...
        translate(rx, CHAO_Y, rz); ambientMaterial(72, 135, 48); box(1, 10, 1)
        translate(0, -6, 0); ambientMaterial(95, 165, 55); sphere(1.8); pop()
...
def desenha_casa(x, z, tex_roof, esc, rot, tem_ch, noite, sh_cor):
    ...
    pq(0,0,1,   -hw,-hh,hd,  hw,-hh,hd,   hw,hh,hd,   -hw,hh,hd)
    pq(0,0,-1,  hw,-hh,-hd,  -hw,-hh,-hd, -hw,hh,-hd, hw,hh,-hd)
    pq(1,0,0,   hw,-hh,hd,   hw,-hh,-hd,  hw,hh,-hd,  hw,hh,hd)
    pq(-1,0,0,  -hw,-hh,-hd, -hw,-hh,hd,  -hw,hh,hd,  -hw,hh,-hd)
    ...
    if tem_ch:
        push(); translate(35, -60, -20); ambientMaterial(188, 118, 88); box(14, 38, 14)
        translate(0, -19, 0); ambientMaterial(168, 98, 68); box(17, 5, 17); pop()

    push(); translate(0, hh-2, hd+5); ambientMaterial(185, 142, 102); box(38, 4, 10); pop()
    ...
    translate(7, 0, 2); ambientMaterial(255, 215, 45); sphere(2); pop()
...
def _jan(x, y, z, noite, sh_cor):
    ...
    if noite: emissiveMaterial(255, 190, 40); ambientMaterial(255, 190, 40)
    else: ambientMaterial(168, 222, 240)
    box(18, 18, 2); translate(0, 0, 1); ambientMaterial(255); emissiveMaterial(0, 0, 0)
    box(18, 2.5, 1); box(2.5, 18, 1)

    ambientMaterial(*sh_cor)
    push(); translate(-14, 0, -1); box(4, 22, 1.5); pop()
    push(); translate(14, 0, -1); box(4, 22, 1.5); pop()
...
def desenha_coelhinho():
    ...
    ambientMaterial(255, 208, 218)
    push(); scale(1,1.15,1); sphere(9); pop() 

    push(); translate(0, -14, 0); ambientMaterial(255, 215, 222); sphere(7.5) 
    push(); translate(-3.5,-9,-1); rotateZ(-0.15)
    ambientMaterial(255,208,218); box(3.8,13,2); ambientMaterial(255,158,178); box(2,9,1); pop()
    push(); translate(3.5,-9,-1); rotateZ(0.15)
    ambientMaterial(255,208,218); box(3.8,13,2); ambientMaterial(255,158,178); box(2,9,1); pop()

    ambientMaterial(25,25,25)
    push(); translate(-3,0,6); sphere(1.8); pop()
    push(); translate(3,0,6); sphere(1.8); pop()
    
    ambientMaterial(255)
    push(); translate(-2.3,-0.8,7); sphere(0.7); pop()
    push(); translate(3.7,-0.8,7); sphere(0.7); pop()

    ambientMaterial(255,140,160); push(); translate(0,2.5,6.5); sphere(1.2); pop() 
    fill(255,162,182,80); push(); translate(-5,2,4); sphere(2); pop(); push(); translate(5,2,4); sphere(2); pop()
    pop()

    push(); translate(0,-2,-9); ambientMaterial(255,242,248); sphere(3.5); pop() 

    ambientMaterial(255,192,202) 
    push(); translate(-5,8,2); box(4,3,5); pop(); push(); translate(5,8,2); box(4,3,5); pop()
...
def desenha_arvore(x, z, h):
    ...
    ambientMaterial(135,92,55); push(); translate(0,-h*0.3,0); box(5,h*0.55,5); pop()
...
def desenha_flores_cluster(x, z):
    ...
    for i in range(4):
        ...
        ambientMaterial(*cores[i%len(cores)]); push(); translate(0,-6.5,0); sphere(2.5)
        ambientMaterial(255,255,198); sphere(1); pop(); pop()
    pop()
...
def desenha_peixe(p):
    ...
    push(); scale(0.8, 0.4, 0.4); sphere(p.tam); pop()
    ...
    fill(255); push(); translate(p.tam*0.4,-1,p.tam*0.25); sphere(0.8); fill(0); sphere(0.4); pop(); pop()
...
def desenha_abelha(b, noite):
    ...
    if noite:
        ...
        push(); scale(1.2, 0.8, 0.8); sphere(1.5); pop()
        ambientMaterial(30, 30, 30)
        push(); scale(0.5, 0.85, 0.85); sphere(1.55); pop()
...
def desenha_fumaca_3d():
    for p in fumaca:
        ...
        alpha_base = 125 if p.tipo == "normal" else 40
        fill(198, 198, 198, int(p.vida * alpha_base))
        sphere(p.tam)
        pop()
...
def desenha_hud(noite, alt_sol):
    ...
    sl_x = width - 55
    fill(0,0,0,40); rectMode(CENTER); rect(sl_x, height/2, 8, 240, 4)
    ...
    triangle(25, height-80, 40, height-90, 40, height-70) 
    triangle(75, height-80, 60, height-90, 60, height-70) 
    fill(255); circle(50, height - 80, 25)
...
def cor_mix(c1, c2, t): return (c1[0]+(c2[0]-c1[0])*t, c1[1]+(c2[1]-c1[1])*t, c1[2]+(c2[2]-c1[2])*t)
